...
const std::map<const u32, const std::string> list_minifigs = {{0, "Blank Tag"},
                                                              {1, "Batman"},
                                                              {2, "Gandalf"},
                                                              {3, "Wyldstyle"},
                                                              {4, "Aquaman"},
...
minifig_creator_dialog::minifig_creator_dialog(QWidget* parent) : QDialog(parent) {
    ...
    connect(btn_create, &QAbstractButton::clicked, this, [=, this]() {
        ...
        if (!ok_num) {
            QMessageBox::warning(this, tr("Error converting value"),
                                 tr("Figure number entered is invalid!"), QMessageBox::Ok);
            return;
        }
        ...
        m_file_path = QFileDialog::getSaveFileName(
            this, tr("Create Figure File"), s_last_figure_path, tr("Dimensions Figure (*.bin);;"));
...
minifig_move_dialog::minifig_move_dialog(QWidget* parent, u8 old_index) : QDialog(parent) {
...
void minifig_move_dialog::add_minifig_position(QGridLayout* grid_panel, u8 index, u8 row, u8 column,
                                               u8 old_index) {
    ...
    connect(btn_move, &QAbstractButton::clicked, this, [this, index] {
        m_index = index;
        m_pad = index == 1 ? 1 : index == 0 || index == 3 || index == 4 ? 2 : 3;
        accept();
    });
...
dimensions_dialog::dimensions_dialog(QWidget* parent, std::shared_ptr<IpcClient> ipc_client)
    : QDialog(parent), m_ipc_client(ipc_client) {
    ...
    hbox_group_1->addSpacerItem(
        new QSpacerItem(50, 0, QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding));
    ...
    hbox_group_2->addSpacerItem(
        new QSpacerItem(50, 0, QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding));
    add_minifig_slot(hbox_group_2, 3, 5);
    add_minifig_slot(hbox_group_2, 3, 6);

    vbox_group->addLayout(hbox_group_1);
    vbox_group->addSpacerItem(
        new QSpacerItem(0, 20, QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding));
...
dimensions_dialog* dimensions_dialog::get_dlg(QWidget* parent,
    ...
    if (inst == nullptr)
        inst = new dimensions_dialog(parent, ipc_client);

    return inst;
}

void dimensions_dialog::clear_all() {
...
void dimensions_dialog::add_minifig_slot(QHBoxLayout* layout, u8 pad, u8 index) {
    ...
    connect(clear_btn, &QAbstractButton::clicked, this,
            [this, pad, index] { clear_figure(pad, index); });
    connect(create_btn, &QAbstractButton::clicked, this,
            [this, pad, index] { create_figure(pad, index); });
    connect(load_btn, &QAbstractButton::clicked, this,
            [this, pad, index] { load_figure(pad, index); });
...
void dimensions_dialog::clear_figure(u8 pad, u8 index) {
    if (figure_slots[index]) {
        m_ipc_client->removeDimensionFigure(pad, index, true);
...
void dimensions_dialog::load_figure(u8 pad, u8 index) {
    const QString file_path = QFileDialog::getOpenFileName(
        this, tr("Select Dimensions File"), s_last_figure_path, tr("Dimensions Figure (*.bin);;"));
...
void dimensions_dialog::move_figure(u8 pad, u8 index) {
    ...
    m_ipc_client->tempRemoveDimensionFigure(index);
    if (move_dlg.exec() == Accepted) {
        m_ipc_client->moveDimensionFigure(move_dlg.get_new_pad(), move_dlg.get_new_index(), pad,
                                          index);
        ...
        m_ipc_client->cancelRemoveDimensionFigure(index);
    }
}

void dimensions_dialog::load_figure_path(u8 pad, u8 index, const QString& path) {
    Common::FS::IOFile dim_file(path.toStdString(), Common::FS::FileAccessMode::ReadWrite);
    if (!dim_file.IsOpen()) {
        QMessageBox::warning(
            this, tr("Failed to open the figure file!"),
    ...
    if (dim_file.Read(data) != data.size()) {
        QMessageBox::warning(
            this, tr("Failed to read the figure file!"),
...
std::array<u8, 16> dimensions_dialog::GenerateFigureKey(const std::array<u8, 0x2D * 0x04>& buf) {
...
std::array<u8, 4> dimensions_dialog::DimensionsRandomize(const std::vector<u8>& key, u8 count) {
    u32 scrambled = 0;
...
u32 dimensions_dialog::Scramble(const std::array<u8, 7>& uid, u8 count) {
...
std::array<u8, 8> dimensions_dialog::Decrypt(const u8* buf, std::optional<std::array<u8, 16>> key) {
...
u32 dimensions_dialog::GetFigureId(const std::array<u8, 0x2D * 0x04>& buf) {
