...
skylander_creator_dialog::skylander_creator_dialog(QWidget* parent) : QDialog(parent) {
...
skylander_dialog::skylander_dialog(QWidget* parent, std::shared_ptr<IpcClient> ipc_client)
...
skylander_dialog* skylander_dialog::get_dlg(QWidget* parent,
    ...
    if (inst == nullptr)
        inst = new skylander_dialog(parent, ipc_client);
...
void skylander_dialog::clear_skylander(u8 slot) {
    if (const auto& slot_infos = sky_slots[slot]) {
        ...
        m_ipc_client->removeSkylander(slot);
        sky_slots[slot] = {};
        update_edits();
    }
}
...
void skylander_dialog::load_skylander_path(u8 slot, const QString& path) {
    Common::FS::IOFile sky_file(path.toStdString(), Common::FS::FileAccessMode::ReadWrite);
    if (!sky_file.IsOpen()) {
        QMessageBox::warning(
    ...
    if (sky_file.Read(data) != data.size()) {
        QMessageBox::warning(
    ...
    u16 sky_id = u16(data[0x11]) << 8 | u16(data[0x10]);
    u16 sky_var = u16(data[0x1D]) << 8 | u16(data[0x1C]);

    sky_file.Close();

    m_ipc_client->loadSkylander(path.toStdString(), slot);
