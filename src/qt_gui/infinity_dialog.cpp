...
figure_creator_dialog::figure_creator_dialog(QWidget* parent, u8 slot) : QDialog(parent) {
    ...
    auto populate_combo = [=, this](int series_filter) -> u32 {
        ...
        for (const auto& [figure, entry] : list_figures) {
            ...
            if ((slot == 0 && ((figure > 0x1E8480 && figure < 0x2DC6BF) ||
    ...
    connect(btn_create, &QAbstractButton::clicked, this, [=, this]() {
        ...
        if (!ok_series || series > 3 || series < 1) {
            QMessageBox::warning(this, tr("Error converting value"),
                                 tr("Series number entered is invalid!"), QMessageBox::Ok);
            return;
        }
        ...
        m_file_path = QFileDialog::getSaveFileName(
            this, tr("Create Figure File"), s_last_figure_path, tr("Infinity Figure (*.bin);;"));
...
bool figure_creator_dialog::create_blank_figure(u32 character, u8 series) {
    ...
    std::array<u8, 16> uid_data = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                   0x00, 0x89, 0x44, 0x00, 0xC2};
    for (u8 i = 0; i < 7; i++) {
        u8 random = rand() % 255;
    ...
    sha1::SHA1 sha;
    sha.processBytes(sha1_calc.data(), sha1_calc.size());
    sha.getDigestBytes(hash);

    u8 key[0x10];
    for (int i = 0; i < 4; i++) {
        for (int x = 0; x < 4; x++) {
            key[x + (i * 4)] = hash[(3 - x) + (i * 4)];
        }
    }

    // Create AES Encrypt context based on AES key, use this to encrypt the character data and 4
    // blank blocks
    std::array<u8, 16> encrypted_block{};
    std::array<u8, 16> encrypted_blank{};
    aes::encrypt_ecb(figure_data.data(), figure_data.size(), &key[0], 16, encrypted_block.data(),
                     encrypted_block.size(), false);
    aes::encrypt_ecb(BLANK_BLOCK.data(), BLANK_BLOCK.size(), &key[0], 16, encrypted_blank.data(),
                     encrypted_blank.size(), false);
...
infinity_dialog::infinity_dialog(QWidget* parent, std::shared_ptr<IpcClient> ipc_client)
...
infinity_dialog* infinity_dialog::get_dlg(QWidget* parent, std::shared_ptr<IpcClient> ipc_client) {
    if (inst == nullptr)
        inst = new infinity_dialog(parent, ipc_client);
...
void infinity_dialog::add_figure_slot(QVBoxLayout* vbox_group, QString name, u8 slot) {
    ...
    connect(clear_btn, &QAbstractButton::clicked, this, [this, slot] { clear_figure(slot); });
    connect(create_btn, &QAbstractButton::clicked, this, [this, slot] { create_figure(slot); });
    connect(load_btn, &QAbstractButton::clicked, this, [this, slot] { load_figure(slot); });
...
void infinity_dialog::clear_figure(u8 slot) {
    if (figure_slots[slot]) {
        m_ipc_client->removeInfinityFigure(slot);
        figure_slots[slot] = 0;
        m_edit_figures[slot]->setText(tr("None"));
    }
}
...
void infinity_dialog::load_figure(u8 slot) {
    const QString file_path = QFileDialog::getOpenFileName(
        this, tr("Select Infinity File"), s_last_figure_path, tr("Infinity Figure (*.bin);;"));
...
void infinity_dialog::load_figure_path(u8 slot, const QString& path) {
    Common::FS::IOFile inf_file(path.toStdString(), Common::FS::FileAccessMode::ReadWrite);
    if (!inf_file.IsOpen()) {
        QMessageBox::warning(
    ...
    if (inf_file.Read(data) != data.size()) {
        QMessageBox::warning(
    ...
    aes::decrypt_ecb(&data[16], 16, &key[0], 16, infinity_decrypted_block.data(),
                     infinity_decrypted_block.size(), nullptr);

    u32 fignum = u32(infinity_decrypted_block[1]) << 16 | u32(infinity_decrypted_block[2]) << 8 |
    ...
    m_ipc_client->loadInfinityFigure(path.toStdString(), slot);
}
